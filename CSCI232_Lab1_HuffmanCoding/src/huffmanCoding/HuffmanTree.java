...
class HuffmanTree
{
	...
	public void insert(int id, char dd)
	{
		...
		//newNode.frequency = id; // insert data
		//newNode.character = dd;
		if(root==null) // no node in root
			root = newNode;
		else // root occupied
		{
			...
			{
				parent = current;
				if(id < current.frequency) // go left?
				{
					current = current.leftChild;
	...
	{ // (assumes non-empty list)
		...
		while(current.frequency != key) // search for node
		{
			parent = current;
			if(key < current.frequency) // go left?
			{
	...
	private void preOrder(Node localRoot)
	{
		if(localRoot != null)
		{
			System.out.print(localRoot.frequency + " ");
